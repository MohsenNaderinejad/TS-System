...
int main(int argc, char ** argv) {
    ...
    std::string TEACHERS_MAIN_JSON_ADDRESS = ADDRESS_CREATOR(std::vector<std::string>{"data", "MAIN_DATAS", "teachers.json"});
    std::string STUDENTS_MAIN_JSON_ADDRESS = ADDRESS_CREATOR(std::vector<std::string>{"data", "MAIN_DATAS", "students.json"});
    std::string EXAMS_MAIN_JSON_ADDRESS = ADDRESS_CREATOR(std::vector<std::string>{"data", "MAIN_DATAS", "exams.json"});
    std::string QUESTIONS_MAIN_JSON_ADDRESS = ADDRESS_CREATOR(std::vector<std::string>{"data", "EXAM_QUESTIONS", "questions.json"});
    ...
    std::ifstream QUESTIONS_READ_FILE(QUESTIONS_MAIN_JSON_ADDRESS);
    ...
    if (!QUESTIONS_READ_FILE.is_open()) {
        ...
        QUESTIONS_READ_FILE.open(QUESTIONS_MAIN_JSON_ADDRESS, std::ios::out);
        if (!QUESTIONS_READ_FILE.is_open()) {
            std::cerr << "Error: Could not create file 'questions.json'!" << std::endl;
            return 1;
        }
        QUESTIONS_READ_FILE.close();
        QUESTIONS_READ_FILE.open(QUESTIONS_MAIN_JSON_ADDRESS, std::ios::in | std::ios::out);
    }
    ...
    // Extracting JSON informations for FILES to JSON Object -- Student Section // Teacher Section
    json EXAM_JSON_FILE;
    ...
    // Extracing JSON informations for FILES to JSON Object -- Student Section // Teacher Section
    ...
    std::ofstream QUESTION_WRITE_FILE(QUESTIONS_MAIN_JSON_ADDRESS);
